...
getFeaturedStars targetStudio targetYear (ms, _, si, _, _) = concatMap (\ movie -> getStars movie) getMoviesFromStudioYear
 where
    ...
    getMoviesFromStudioYear = [title | (Movie title year _ studio _) <- ms, studio == targetStudio && year == targetYear]

getPresident :: Name -> MovieDB -> Name
getPresident studioName (_, _, _, ss, mes) = getName getPresidentID
 where
    getName targetID = head [presName | (MovieExec presName id _) <- mes, id == targetID]
    getPresidentID = head [id | (Studio name id) <- ss, name == studioName]



-- Typeclasses and algebraic types
