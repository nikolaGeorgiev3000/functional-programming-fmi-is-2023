...
getSunk (outcomes, _, _) = map (\ battle -> (battle, getSunkShips battle)) getBattles
 where
    getSunkShips battleName = [shipName | (Outcome shipName osBattleName result) <- outcomes, result == "sunk" && battleName == osBattleName]
    getBattles = sort $ nub [battleName | (Outcome _ battleName _) <- outcomes]
