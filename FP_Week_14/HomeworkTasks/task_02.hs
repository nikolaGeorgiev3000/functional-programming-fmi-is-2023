...
mapNested f (Elem x)  = Elem (f x)                  -- Apply `f` to the element, and wrap it in an `Elem` constructor.
mapNested f (List xs) = List (map (mapNested f) xs) -- `map` (mapNested with f) over each nested element in the list. 
